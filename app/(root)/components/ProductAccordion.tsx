...
const ProductAccordion = ({ product }: ProductAccordionProps) => {
  ...
  const info = product?.commerceInfo ?? {};
          ...
          <div className="space-y-4">
            ...
            {Number(info.fabricWeight) > 0 && (
              <Detail label="Fabric weight">
                <p>{Number(info.fabricWeight)} GSM</p>
              </Detail>
            )}
          ...
          <div className="space-y-4">
            ...
            {Number(info.withdrawalDays) > 0 && (
              <Detail label="Right of withdrawal">
                <p>{Number(info.withdrawalDays)} days</p>
              </Detail>
            )}
                ...
                <div className="overflow-hidden">
                  <div
                    className={`origin-top pb-5 pt-1 transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
                      isOpen ? "translate-y-0 scale-y-100" : "-translate-y-1 scale-y-[0.98]"
                    }`}
                  >
                    {item.content}
                  </div>
