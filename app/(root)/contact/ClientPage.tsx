  ...
  return (
    <section id={id} className="print:break-before-page text-xs scroll-mt-20">
      ...
      >
        ...
        <div className="px-4 py-4 flex flex-col items-start gap-3 text-mbg-black/95">
          {children}
        </div>
      </details>
    </section>
  ...
  <>
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="m-0 text-[12px] font-semibold uppercase text-mbg-darkgrey">
              Phone
            </p>
    ...
    <Section id="faq" forceOpen={allOpen ?? undefined} title="3. Quick FAQ">
      <details className="w-full rounded-xs border border-mbg-black/7 bg-mbg-white">
        ...
        <div className="px-3 pb-3 text-mbg-black/95 text-xs">
          We send you the tracking link by email as well as your invoice once
          your order is shipped. You can see the progress of your order by going
          to “Orders” page in the basketball icon menu.
        </div>
      </details>
      <details className="w-full rounded-xs border border-mbg-black/7 bg-mbg-white">
        <summary className="cursor-pointer select-none px-3 py-2 text-[12px] font-semibold">
        ...
        <div className="px-3 pb-3 text-mbg-black/95 text-xs">
          Contact support. Returns accepted within 14 days. See{" "}
          <Link className="mbg-link" href="/terms-conditions">
            Terms & Conditions
          </Link>
          .
        </div>
    ...
    <Section
      id="retours"
      forceOpen={allOpen ?? undefined}
      title="4. Returns & support"
    >
      <p>
        For returns/exchanges, include your order number (#) and the reason.For
        more precision see{" "}
        <Link className="mbg-link" href="/terms-conditions">
          Terms & Conditions
        </Link>
...
const ContentFR: React.FC<{ allOpen: boolean | null }> = ({ allOpen }) => (
  <>
    ...
    <Section
      id="horaires"
      forceOpen={allOpen ?? undefined}
      title="2. Horaires & délais"
    >
      <ul className="list-disc space-y-1 pl-6">
        <li>Service client : lundi → vendredi, 9h–12h / 14h–17h (CET).</li>
        <li>
          Expédition : commandes passées avant 14h expédiées le jour même.
        </li>
      </ul>
    </Section>
    ...
    <Section id="faq" forceOpen={allOpen ?? undefined} title="3. FAQ (rapide)">
      <details className="w-full rounded-xs border border-mbg-black/7 bg-mbg-white">
        ...
        <div className="px-3 pb-3 text-mbg-black/95 text-xs">
          Nous vous envoyons un couriel de suivi de votre commande ainsi que
          votre facture une fois que votre commande est posté. Vous pouvez
          suivre la progression de votre commande en vous rendant sur la page
          “Orders” qui se situe dans le menu ayant pour icône un ballon de
          basket.
        </div>
      </details>
      <details className="w-full rounded-xs border border-mbg-black/7 bg-mbg-white">
        ...
        <div className="px-3 pb-3 text-mbg-black/95 text-xs">
          Contacter le service client. Retours sous 14 jours. Voir les{" "}
          <Link className="mbg-link" href="/terms-conditions">
            CGV
          </Link>
          .
    ...
    >
      <p>
        Pour les retours/échanges, indiquez votre n° de commande ainsi que le
        motif. Pour plus de précision voir les{" "}
        <Link className="mbg-link" href="/terms-conditions">
          CGV
        </Link>
        .
  ...
  const ui = useMemo(
    () => ({
      ...
      intro:
        lang === "fr" ? (
          <>
            Des questions sur une commande, un retour ou un vêtement fait main ?
            Écrivez‑nous, nous répondons généralement sous 24–48h ouvrées.
          </>
        ) : (
          <>
            Questions about an order, a return, or handmade apparel? Write to
            us, we usually reply within 24–48 business hours.
          </>
        ),
      lastUpdateLabel:
